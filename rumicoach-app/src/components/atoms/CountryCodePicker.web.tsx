...
export function CountryCodePicker({ value, onChange, style }: CountryCodePickerProps) {
  ...
  const triggerText = selectedCountry
    ? `${selectedCountry.flag}  ${selectedCountry.code}  ${selectedCountry.phoneCode}`
    : ''
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      const portal = document.querySelector('[data-country-code-dropdown]')
  ...
  return (
    <View ref={inputRef}>
      <ThemedInput
        value={showDropdown ? search : triggerText}
        onFocus={() => {
          if (blurTimeoutRef.current) { clearTimeout(blurTimeoutRef.current); blurTimeoutRef.current = null }
          openDropdown()
        }}
        ...
        variant='light'
        fontSize={15}
        style={[{ height: 48, width: 120 } as ViewStyle, ...(style ? [style] : [])]}
      />
      {showDropdown && dropdownPos && typeof document !== 'undefined' && createPortal(
        <div
          data-country-code-dropdown
          style={{
            position: 'fixed',
            top: dropdownPos.top,
            left: dropdownPos.left,
            width: dropdownPos.width + 40,
            maxHeight: 200,
            overflowY: 'auto',
            borderRadius: 12,
          ...
          {filteredCountries.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ color: '#1a1a1a', fontSize: 14, fontWeight: isSelected ? '600' : '400' }}>
                    {item.name}
                  </div>
